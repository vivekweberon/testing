...
const Banner = ({ bannerProps, formsets }) => {
  ...
  return (
    <>
      <Head>
        <link rel="stylesheet" href="css/components/Banner.css" onError={(e) => logResourceLoadError(e.target)} />
      </Head>
      <div className="header-container" ref={headerContainerRef} role="banner">
        <div className="butterBarContainer"></div>
        <header className="wrapper clearfix bgclr8" style={bannerImageBorder ? { borderTop: `${bannerImageBorder}` } : null}>
          <div className="praveenImg">
            <div id="pp" className="pp">
              {bannerProps.imgOverlay && <img src={globalConfig["DEFAULT-PRAVEEN_IMG_SRC"]} width="100%" alt="praveen image" onError={(e) => logResourceLoadError(e.target)} />}
            </div>

            <div className="bannerTextContainer">
              <h1 className="usp">
                <div>{parse(bannerProps.text)}</div>
              </h1>
            </div>
          </div>
